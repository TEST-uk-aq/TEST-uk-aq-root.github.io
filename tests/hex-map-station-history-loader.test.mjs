...
const observationHistory = section.indexOf("const observationPromise");
...
assert.match(page, /await commit\(await settle\(work\[0\]\)\)/, "the newest missing range settles and commits before older parallel work starts");
...
assert.match(page, /Promise\.all\(\[aqiPromise, observationPromise\]\)/, "AQI and observation streams remain independent without an unbounded chunk Promise.all");
